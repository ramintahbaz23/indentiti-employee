        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="11" style="text-align: center; padding: 3rem;">
                    <div style="color: #706e6b;">No estimates found matching your criteria.</div>
                </td>
            </tr>
        `;
        renderPagination();
...
function updateTotalCount() {
    ...
    if (countElement) {
        countElement.textContent = `${count} ${count === 1 ? 'estimate' : 'estimates'}`;
    }
}
...
function renderPagination() {
    ...
    if (infoEl) {
        infoEl.textContent = total === 0 ? '0 estimates' : `${start}–${end} of ${total} estimates`;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof estimatesData !== 'undefined' && estimatesData && Array.isArray(estimatesData)) {
        ...
        state.filteredEstimates = estimatesData;
    } else {
        console.error('estimatesData is not defined or is not an array');
        state.estimates = [];
        state.filteredEstimates = [];
    }
...
function downloadEstimate(estimateId) {
    const estimate = state.estimates.find(est => est.id === estimateId);
    if (estimate) {
        alert(`Downloading estimate ${estimate.workOrderNumber}`);
